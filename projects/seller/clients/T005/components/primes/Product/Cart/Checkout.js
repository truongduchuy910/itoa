...
const Checkout = ({
   ...
   *
   */
  var { data, error, loading } = useQuery(INFORMATION_QUERY, {
        ...
        >
          ...
          <Box>
            ...
            <Text size={"xs"} color={"gray.500"} mb={3}>
              Vui lòng cung cấp thêm thông tin khi không thể liên hệ bạn bằng
              điện thoại. Ví dụ: Instagram, Facebook, Gmail,...
            </Text>
        ...
        >
          ...
          <div className="box my-3">
            ...
            <Box>
              ...
              <Text style={{ float: "right" }}>
                <Field>{total - page.shipMoneySupport}</Field> ₫
              </Text>
            </Box>
            <Box mb={5}>
              Vận chuyển:{" "}
          ...
          <Button
            onClick={(e) => {
              ...
              if (!phone)
                return onToast()(`Vui lòng điền số điện thoại, cảm ơn!`);
              if (!name) return onToast()(`Vui lòng điền thọ tên, cảm ơn!`);
              if (!addr) return onToast()(`Vui lòng điền địa chỉ, cảm ơn!`);
              if (!note)
                return onToast()(`Vui lòng điền thông tin ghi chú, cảm ơn!`);
              onOrder({
                phone,
                name,
                address: addr,
...
const Address = ({ onChangeAddress = (address) => {} }) => {
  ...
  if (!previousData && (loading || error))
    return <Loading style={{ minHeight: 120 }} />;
  const { allGhnProvinces, allGhnDistricts, allGhnWards } =
    data || previousData;
  return (
    <Fragment>
      <Select
