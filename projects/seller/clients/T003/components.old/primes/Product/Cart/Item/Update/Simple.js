    ...
    <Box py={1} mb={3}>
      <Flex borderWidth="1px" borderRadius="lg">
        <Box flex={4} display={'flex'} justifyContent={'center'} alignItems={'center'}>
          <Box>
            <Image src={productCartItem.product?.image.publicUrl} />
          </Box>
        </Box>
        <Box flex={6} p={2} display={'flex'} justifyContent={'center'} alignItems={'center'}>
          <Box>
            <Stat>
              <StatLabel>
              ...
              <StatNumber>
                {formatMoney(
                  (productCartItem.sale || productCartItem.price) * productCartItem.quantity
                )}{' '}
                ₫
              </StatNumber>
            ...
            {!readOnly ? (
              <Fragment>
                <IconButton icon={<MinusIcon />} size={'xs'} onClick={clickMinus} />
                <Text mx={2} display={'inline-block'}>
                  {quantity}
                </Text>
                <IconButton icon={<AddIcon />} size={'xs'} onClick={clickPlus} />
                <IconButton
                  icon={<DeleteIcon />}
                  size={'xs'}
                  float={'right'}
