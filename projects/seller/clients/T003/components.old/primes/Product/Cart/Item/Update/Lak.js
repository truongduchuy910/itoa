...
export default function ProductCartItemItemUpdateLak({
  ...
  const price = (productCartItem.sale ? productCartItem.sale : productCartItem.price) * quantity;
  return (
    <Fragment>
      <tr key={productCartItem.id} className="cartItem">
        <td className="product">
          <div className="thumb-cart row">
            <a
              // onClick={() => onClickProduct(productCartItem.product.url)}
              title={productCartItem.product.name}
              className="col-12 col-md-4 mt-4"
            >
              <img
                src={`${productCartItem.product?.image?.publicUrl}`}
                alt={productCartItem.product.name}
            ...
            <div className="col">
              ...
              {productCartItem.sale ? (
                <Fragment>
                  <span>{formatMoney(productCartItem.sale)}₫</span>
                  <div className="variant_title old">{formatMoney(productCartItem.price)}₫</div>
                </Fragment>
              ) : (
                <span style={{ marginBottom: 8 }}>{formatMoney(productCartItem.price)}₫</span>
              )}
        ...
        <td className="remove">
          <a title="Xóa" className="remove-item" onClick={onClickRemove}>
            <DeleteIcon />
          </a>
        </td>
      </tr>
