...
const ProductCartItemLakShort = ({ item, deleteProdcutCartItem, refetch, onClickProduct }) => {
    ...
    <tr key={item.id} className="cartItem">
      <td className="product">
        <div className="thumb-cart row">
          ...
          >
            <img
              src={`${process.env.CMS}${item.product?.image?.publicUrl}`}
              alt={item.product.name}
              style={{ marginBottom: 21, width: '100%' }}
            />
          </a>
          <div className="col">
            <a onClick={() => onClickProduct(item.product.url)} title={item.product.name}>
              <h4
                style={{
            ...
            {item.sale ? (
              <Fragment>
                ...
                <div className="variant_title old">{formatMoney(item.price)}₫</div>
              </Fragment>
            ) : (
              <span style={{ marginBottom: 8 }}>{formatMoney(item.price)}₫</span>
            )}
