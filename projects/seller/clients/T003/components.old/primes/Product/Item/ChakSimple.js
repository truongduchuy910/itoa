import { Box, Center, Heading, Text, Stack, Image, Link } from '@chakra-ui/react';
import { Fragment } from 'react';

import { formatMoney } from '../../../utils/chip';

export default function ProductSimple({ product: { name, price, sale, image, brand }, onClick }) {
  return (
    <Center h={'full'}>
      <Link
        h={'full'}
      ...
      >
        <Box
          ...
          _after={{
            ...
            top: 3,
            left: 0,
            backgroundImage: `url(${image?.publicUrl?.replace(/img/g, 'img-sm')})`,
            filter: 'blur(15px)',
            zIndex: -1,
          }}
          _groupHover={{
          ...
          <Stack align={'center'}>
            {sale ? (
              <Fragment>
                ...
                <Text textDecoration={'line-through'}>{formatMoney(price)} ₫</Text>
              </Fragment>
            ) : (
              <Text fontWeight={800} fontSize={'sm'} color={'brand.500'}>
                {formatMoney(price)} ₫
