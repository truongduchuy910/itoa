...
import ProductCartItemList from '../../primes/Product/Cart/Item/List';
import Count from '../../primes/Product/Cart/Item/List/Count';
import { useEffect, useRef } from 'react';
import { makeVar } from '@apollo/client';
const ProductCartItemListSimple = dynamic(() =>
  import('../../primes/Product/Cart/Item/List/Simple')
);
export const onOpenCartVar = makeVar(() => {});
export const onCloseCartVar = makeVar(() => {});
        ...
        <Flex minH={'60px'} w={'full'} py={{ base: 2 }} align={'center'}>
          ...
          <Flex flex={{ base: 1 }} justify={{ base: 'center', md: 'start' }}>
            <Link>
              ...
              >
                <img src={page.logo?.publicUrl} style={{ height: 50 }} /> {/* {page.store} */}
              </Text>
            </Link>
          ...
          <Stack flex={{ base: 1, md: 0 }} justify={'flex-end'} direction={'row'} spacing={2}>
            <IconButton icon={<AiOutlineSearch />} onClick={onClickSearch} />
            <Button
              leftIcon={<AiOutlineShopping style={{ paddingBottom: 2 }} />}
              colorScheme="gray"
              variant="solid"
              ref={btnRefCart}
              onClick={onOpenCart}
    ...
    <Stack direction={'row'} spacing={4} justifyContent="center" alignItems="center">
      {allProductCategories &&
        allProductCategories.map(category => {
          return (
            <Popover key={category.id} trigger={'hover'} placement={'bottom-start'}>
              <PopoverTrigger>
                <Link
                  p={2}
                  fontSize={'sm'}
...
const MobileNav = ({
  ...
  return (
    <Stack p={4} display={{ md: 'none' }}>
      ...
      {allPostHashtags?.length ? (
        <MobileNavItem items={allPostHashtags} name={'Bài viết'} onClickItem={onClickPostHashtag} />
      ) : null}
    </Stack>
  );
};
...
const MobileNavItem = props => {
  const { items = [], name, onClickItem = item => {} } = props;
  const { isOpen, onToggle } = useDisclosure();
  return (
    <Stack spacing={4} onClick={items?.length ? onToggle : () => onClickItem(props)}>
      <Flex
        py={2}
        as={Link}
