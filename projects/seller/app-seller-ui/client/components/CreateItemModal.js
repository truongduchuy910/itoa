...
import { arrayToObject, captureSuspensePromises, countArrays, mapKeys, omitBy } from '@itoa/utils';
      ...
      footer={
        <Fragment>
          <LoadingButton
            ...
            type="submit"
          >
            {hasWarnings && !hasErrors ? 'Bỏ qua cảnh báo và tạo' : 'Tạo'}
          </LoadingButton>
          <Button className="btn btn-warning" variant="subtle" onClick={_onClose}>
            Hủy
          </Button>
        </Fragment>
      }
            ...
            <Render>
              {() => {
                ...
                return creatable.map((field, i) => (
                  <div
                    key={'captureSuspensePromises' + i}
                    className={(field.config?.className || 'col-12') + ' float-left px-1'}
                  >
...
const ConfirmModal = ({ isOpen, onConfirm, onCancel }) => {
  return (
    <Confirm isOpen={isOpen}>
      <p style={{ marginTop: 0 }}>Tất cả dữ liệu trong form sẽ bị hủy bỏ. Bạn chắc chứ?</p>
      <footer>
        <Button className="btn btn-danger" variant="ghost" onClick={onConfirm}>
          Hủy bỏ
        </Button>
