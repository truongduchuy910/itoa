...
import { mapKeys, arrayToObject, omitBy, captureSuspensePromises, countArrays } from '@itoa/utils';
...
const ItemDetails = ({ list, item: initialData, itemErrors, onUpdate }) => {
  ...
  const onSave = async () => {
    ...
    const mutationResult = await updateItem({
            ...
            {getRenderableFields(list).map((field, i) => {
              ...
              return (
                <div
                  key={i}
                  className={
                    (field.config?.className || (isReadOnly ? 'col-6' : 'col-12')) +
                    ' float-left px-1'
                  }
                >
