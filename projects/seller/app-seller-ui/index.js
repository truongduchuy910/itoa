...
class AdminUIApp {
  constructor({
    ...
    if (authStrategy && authStrategy.authType !== 'password') {
      throw new Error('Itoa 5 Admin currently only supports the `PasswordAuthStrategy`');
    }
  ...
  build({ itoa, distDir }) {
    ...
    const adminMeta = this.getAdminUIMeta(itoa);
    const compilers = [];
    const secureCompiler = webpack(
      getWebpackConfig({
        adminMeta,
        adminViews: this.getAdminViews({ itoa, includeLists: true }),
        entry: 'index',
    ...
    if (this.authStrategy) {
      const publicCompiler = webpack(
        getWebpackConfig({
          ...
          adminViews: this.getAdminViews({ itoa, includeLists: false }),
          entry: 'public',
  ...
  getAdminUIMeta(itoa) {
    ...
    const { appId, pageId, name, adminPath, apiPath, graphiqlPath, pages, hooks } = this;
    const { signinPath, signoutPath } = this.routes;
    const { lists } = itoa.getAdminMeta({ schemaName: this._schemaName });
  ...
  getAdminViews({ itoa, includeLists }) {
    const { pages, hooks } = this;
    const { listViews } = includeLists
      ? itoa.getAdminViews({ schemaName: this._schemaName })
      : { listViews: {} };

    return { pages, hooks, listViews };
  }

  prepareMiddleware({ itoa, distDir, dev }) {
    ...
    app.use(adminPath, (req, res, next) => {
      ...
      res.format({
        ...
        '*/*': () => {
          ...
          // See.. https://github.com/itoa-vn/itoa/issues/2741
    ...
    const middlewarePairs = dev
      ? this.createDevMiddleware({ adminMeta: this.getAdminUIMeta(itoa), itoa })
  ...
  createProdMiddleware({ distDir }) {
    ...
    if (!fs.existsSync(builtAdminRoot)) {
      throw new Error(
        'There are no Admin UI build artifacts. Please run `itoa build` before running `itoa start`'
      );
    }
  ...
  createDevMiddleware({ adminMeta, itoa }) {
