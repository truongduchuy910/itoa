...
const { Integer } = require('@itoa/fields');
...
The `implementation` and `adapters` reference back-end code used by Itoa, and everything under `views`, references front-end code used in either the Admin UI or GraphQL apps.

You may have realised now that front-end and back-end code cannot be bundled together in the same file. That's why we use `require.resolve` to provide a string value rather than importing front-end code. The string value is a reference to the location of the files. Itoa has a special build step that compiles front-end code for field types.
...
The implementation class is used in the Itoa back-end. It defines a number of things. Firstly, the GraphQL interface including types, queries and resolvers; secondly, field properties such as if a field is orderable; and finally, what data Itoa should pass to the Admin UI.
...
**Note**: `Integer.views.Controller` will also resolve to a path within the `node_modules` folder and will also be bundled by Itoa at build time.
...
You can find an example of this component in the [custom-fields demo project](https://github.com/itoa-vn/itoatree/main/examples/custom-fields/fields/Stars/views/Stars.js).

Once we have a component for star ratings we can use this in Itoa.

#### Cell

The cell component is rendered in a table that shows a list of items the in Itoa Admin UI. We're going to use the star component we made. Our cell component is going to make use of the data, which will be the integer value, and the `starCount` field config option we added in the Implementation.js file. You can interact with the cell so we don't provide any `onChange` prop to the component.
...
The field component provides the main interface used when creating or editing list items in Itoa. To get a consistent look, we're going to import some wrapping components from `@arch-ui/fields`, that's Itoa's UI library. This will render the labels and provided consistent spacing. Other than this we simply delegate the `onChange` event to the `Stars` so that values can be updated when a user clicks on the stars.
...
Take a look at this example and others in the [custom-fields demo project](https://github.com/itoa-vn/itoatree/main/examples/custom-fields/) in the Itoa repository.
