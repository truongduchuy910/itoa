...
const { objMerge } = require('@itoa/utils');

const FAKE_ID = { mongoose: '5b3eabd9e9f2e3e4866742ea' };
const FAKE_ID_2 = { mongoose: '5b3eabd9e9f2e3e4866742eb' };
...
function setupItoa(adapterName) {
  return setupServer({
    ...
    createLists: itoa => {
      ...
      itoa.createAuthStrategy({ type: PasswordAuthStrategy, list: 'User' });

      listAccessVariations.forEach(access => {
        itoa.createList(getStaticListName(access), {
          fields: {
            name: { type: Text },
        ...
        itoa.createList(getImperativeListName(access), {
          fields: {
            name: { type: Text },
        ...
        itoa.createList(getDeclarativeListName(access), {
