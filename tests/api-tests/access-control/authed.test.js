...
multiAdapterRunners().map(({ before, after, adapterName }) =>
  describe(`Adapter: ${adapterName}`, () => {
    let itoa, items, user;
    beforeAll(async () => {
      const _before = await before(setupItoa);
      itoa = _before.itoa;
      ...
      items = {};
      const context = itoa.createContext({ schemaName: 'internal' });
          ...
          listAccessVariations
            ...
            .forEach(access => {
              test(`allowed: ${JSON.stringify(access)}`, async () => {
                ...
                const { data, errors } = await itoa.executeGraphQL({ query });
                expect(errors).toBe(undefined);
    ...
    describe('read', () => {
      test('authed user', async () => {
        ...
        const context = itoa.createContext({ authentication: { item: user, listKey: 'User' } });
        const { data, errors } = await itoa.executeGraphQL({ context, query });
