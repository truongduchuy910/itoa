...
import { createItem, deleteItem, updateItem } from '@itoa/server-side-graphql-client';
import { multiAdapterRunners, setupServer } from '@itoa/test-utils';
import { Text, Slug } from '@itoa/fields';
...
const setupList = (adapterName, fields) => () =>
  setupServer({
    adapterName,
    createLists: itoa => {
      itoa.createList(generateListName(), { fields });
    },
  });
    ...
    describe(`Adapter: ${adapterName}`, () => {
      ...
      test(
        ...
        runner(
          ...
          async ({ itoa }) => {
            const { key: listKey } = itoa.listsArray[0];
            const item = { name: 'Test Entry!', title: 'A title' };
            const returnFields = 'url';
            const result = await createItem({ itoa, listKey, item, returnFields });
      ...
      test(
        ...
        runner(
          ...
          async ({ itoa }) => {
            ...
            const result = await createItem({ itoa, listKey, item, returnFields: 'url' });
            expect(result).toMatchObject({ url: 'foobar' });
          }
        )
      );
        ...
        test(
          ...
          runner(
            ...
            async ({ itoa }) => {
              ...
              const returnFields = 'url';
              await createItem({ itoa, listKey, item });
              const { url } = await createItem({ itoa, listKey, item, returnFields });
        ...
        test("Doesn't call 'makeUnique' when isUnique: false & a collision occurs", () => {
          ...
          return runner(
            ...
            async ({ itoa }) => {
              ...
              const item = { name: 'Awesome Sauce', url: 'awesome-sauce' };
              const returnFields = 'url';
              await createItem({ itoa, listKey, item, returnFields });
              const result = await createItem({ itoa, listKey, item, returnFields });
        ...
        it('Calls generate with `{ resolvedData, existingData }` on update', () => {
          ...
          return runner(
            ...
            async ({ itoa }) => {
              const { key: listKey } = itoa.listsArray[0];
              const item = { name: 'Awesome Sauce', url: 'awesome-sauce' };
              const { id } = await createItem({ itoa, listKey, item });
        ...
        test(
          ...
          runner(
            ...
            async ({ itoa }) => {
              ...
              const { id, url } = await createItem({ itoa, listKey, item, returnFields });
        ...
        test(
          ...
          runner(
            ...
            async ({ itoa }) => {
              ...
              const item = { name: 'Hi Ho', url: 'hi-ho' };
              const returnFields = 'id url';
              const originalItem = await createItem({ itoa, listKey, item });
              const { id, url } = await createItem({ itoa, listKey, item, returnFields });
              ...
              await deleteItem({ itoa, listKey, itemId: originalItem.id });
        ...
        test(
          ...
          runner(
            ...
            async ({ itoa }) => {
              ...
              await createItem({ itoa, listKey, item: item1 });
              const { id } = await createItem({ itoa, listKey, item: item2 });
              const result = await updateItem({
                itoa,
                listKey,
