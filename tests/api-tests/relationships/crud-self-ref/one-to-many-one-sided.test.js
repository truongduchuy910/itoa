...
const createComplexData = async itoa => {
  const { data, errors } = await itoa.executeGraphQL({
    query: `
    mutation {
      createUsers(data: [
  ...
  const {
    data: { allUsers },
    errors: errors2,
  } = await itoa.executeGraphQL({ query: '{ allUsers { id name friend { id name } } }' });
    ...
    describe(`One-to-many relationships`, () => {
      ...
      describe('Update', () => {
        ...
        test(
          ...
          runner(setupItoa, async ({ itoa }) => {
            // Manually setup a connected Company <-> Location
            const { friend, user } = await createUserAndFriend(itoa);
        ...
        ['A', 'B', 'C', 'D', 'E'].forEach(name => {
          test(
            ...
            runner(setupItoa, async ({ itoa }) => {
              ...
              const { users } = await createComplexData(itoa);
