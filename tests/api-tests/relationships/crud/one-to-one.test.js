...
const { createItem, getItems, getItem } = require('@itoa/server-side-graphql-client');
...
const createLocationAndCompany = async itoa => {
  const {
    data: { createLocation },
    errors,
  } = await itoa.executeGraphQL({
    ...
    describe(`One-to-one relationships`, () => {
      describe('Read', () => {
        ...
        test(
          ...
          runner(setupItoa, async ({ itoa }) => {
            await createInitialData(itoa);
            const { location, company } = await createLocationAndCompany(itoa);
            const { data, errors } = await itoa.executeGraphQL({
              query: `{
                  allLocations(where: { company: { name: "${company.name}"} }) { id }
                  allCompanies(where: { location: { name: "${location.name}"} }) { id }
        ...
        if (adapterName !== 'mongoose') {
          test(
            ...
            runner(setupItoa, async ({ itoa }) => {
              await createInitialData(itoa);
              await createCompanyAndLocation(itoa);
              const { data, errors } = await itoa.executeGraphQL({
                query: `{
          ...
          test(
            ...
            runner(setupItoa, async ({ itoa }) => {
              await createInitialData(itoa);
              await createLocationAndCompany(itoa);
              const { data, errors } = await itoa.executeGraphQL({
                query: `{
                  allLocations(where: { company_is_null: true }) { id }
                  allCompanies(where: { location_is_null: true }) { id }
