const { sizes, public_path } = require('../../schemas/stores');
...
function handler(req, res) {
  ...
  const size =
    sizes.sort((a, b) => (a.size > b.size ? -1 : 1)).find(a => a.size <= (w <= h ? w : h)) ||
    sizes[0];
  ...
  else res.sendFile(path.join(path.resolve(), public_path, '/assets/img/no-image.png'));
}
