...
Different contexts are supported by different file adapters. This package contains the built-in file adapters supported by ItoaJS.
...
> **Note 2:** You may also need to use a [static file server](https://v5.itoa.vn/itoa/app-static/#static-file-app) to host the uploaded files.
...
```js
const { File } = require('@itoa/fields');
...
```javascript
const { CloudinaryAdapter } = require('@itoa/file-adapters');
