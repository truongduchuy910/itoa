...
class GraphQLApp {
   ...
   */
  prepareMiddleware({ itoa, dev }) {
    const server = itoa.createApolloServer({
    ...
    if (dev && graphiqlPath) {
      ...
      app.use(new GraphQLPlaygroundApp({ apiPath, graphiqlPath }).prepareMiddleware({ itoa, dev }));
    }
    ...
    // { cors: false } - prevent ApolloServer from overriding Itoa's CORS configuration.
