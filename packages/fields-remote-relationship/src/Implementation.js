...
import { Implementation } from '@itoa/fields/src/Implementation';
import { resolveNested } from './nested-mutations';

export class Relationship extends Implementation {
  constructor(path, { ref, many, withMeta, file, search }) {
    super(...arguments);
    ...
    this.config = { ...this.config, file, search };
    this.adminConfig = { ...this.adminConfig, file, search };
...
export class MongoRelationshipInterface extends MongooseFieldAdapter {
  ...
  // addToMongooseSchema là hàm thể hiện định dạng của field khi thêm vào mongo
  addToMongooseSchema(schema, _mongoose, rels) {
    // Nếu A 1:1 B thì, (refFieldPath 1 : 1 refListKey)
