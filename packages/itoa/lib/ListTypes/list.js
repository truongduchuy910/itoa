...
const { parseListAccess } = require('@itoa/access-control');
const {
...
module.exports = class List {
  constructor(
    ...
    this.adapterName = adapter && adapter.name;
    this.adapter = adapter && adapter.newListAdapter(this.key, adapterConfig);
    ...
    // Tell Itoa about all the types we've seen
  ...
  initFields() {
    ...
    if (!sanitisedFieldsConfig.id && this.adapter) {
    ...
    Object.values(sanitisedFieldsConfig).forEach(({ type }) => {
      if (!type.adapters[this.adapterName] && this.adapter) {
        throw new Error(
  ...
  async listQueryMeta(args, context, gqlName, info, from) {
    return {
      ...
      // Evaluation takes place in ../Itoa/index.js
      getCount: async () => {
        const access = await this.checkListAccess(context, undefined, 'read', { gqlName });
  ...
  getGqlTypes({ schemaName }) {
    ...
    if (
      ...
      const typeDefine = `
      """ ${this.schemaDoc || 'A itoa list'} """
      type ${this.gqlNames.outputTypeName} @key(fields: "id") {
        """
