...
const { arrayToObject, objMerge, flatten, unique, filterValues, upcase } = require('@itoa/utils');
const {
  validateFieldAccessControl,
  validateListAccessControl,
  validateCustomAccessControl,
  validateAuthAccessControl,
} = require('@itoa/access-control');
const { SessionIntergrate } = require('@itoa/session');
...
module.exports = class Itoa {
  constructor({
    ...
    this._schemas = {};
    // getAuthedItem(req, itoa)
    this._sessionManager = new SessionIntergrate(session, authService);
    if (cookieSecret) console.info('Tùy chọn cookieSecret trong phiên bản này đã bị lược bỏ');
    if (cookie) console.info('Tùy chọn cookie trong phiên bản này đã bị lược bỏ');
    if (sessionStore) console.info('Tùy chọn sessionStore trong phiên bản này đã bị lược bỏ');
    ...
    this._listCRUDProvider = new ListCRUDProvider({ metaPrefix: appName });
    ...
    if (adapter) {
      if (gateway) {
        throw new Error('Không thể sử dụng đồng thời adapter và gateway!');
      }
      this.adapter = adapter;
    } else {
      if (gateway) {
        this.gateway = gateway;
      } else throw new Error('Không tìm thấy database nào được cung cấp!');
    }
  ...
  createContext({ schemaName = 'public', authentication = {}, skipAccessControl = false } = {}) {
    ...
    context.excuteExtendGQL = ({ context = defaults.context, query, variables }) => {
  ...
  executeGraphQL({ context, query, variables }) {
    ...
    if (!schema) {
      throw new Error(
        `No executable schema named '${context.schemaName}' is available. Have you setup '@itoa/app-graphql'?`
      );
    }
  ...
  createAuthStrategy(options) {
    const {
      ...
      config,
      hooks,
    } = composePlugins(options.plugins || [])(options, { itoa: this });
    ...
    if (!this.lists[listKey]) {
      strategy.gqlNames = {
        ...
        authenticateMutationName: `authenticate${listKey}With${upcase(strategy.authType)}`,
  ...
  createList(key, config, { isAuxList = false } = {}) {
    ...
    // Itoa automatically adds an 'Upload' scalar type to the GQL schema. Since list output
    ...
    const list = new List(
      key,
      composePlugins(config.plugins || [])(config, { listKey: key, itoa: this }),
      {
        getListByKey,
        adapter,
   ...
   */
  async connect(args) {
    if (this.adapter) await this.adapter.connect({ rels: this._consolidateRelationships() });
  ...
  async _prepareMiddlewares({ dev, apps, distDir, pinoOptions, cors }) {
    return flattenDeep([
      ...
      this._sessionManager.getSessionMiddleware({ itoa: this }),
  ...
  async prepare({
    ...
    ['extendGraphQLSchema', 'createList', 'createAuthStrategy'].forEach(f => {
      this[f] = () => {
        throw new Error(`itoa.${f} must be called before itoa.prepare()`);
      };
    });
