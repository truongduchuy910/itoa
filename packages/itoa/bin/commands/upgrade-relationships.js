...
const strategySummary = (
  ...
  const mongo = itoa.adapter.name === 'mongoose';
...
const upgradeRelationships = async (args, entryFile) => {
  ...
  const { itoa } = require(path.resolve(entryFile));

  const rels = itoa._consolidateRelationships();
  ...
  console.log('');
  ttyLink('https://www.itoa.vn/discussions/relationships', 'More info on relationships');
