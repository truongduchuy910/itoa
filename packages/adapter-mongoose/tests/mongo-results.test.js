...
function getAggregate(database, collection) {
  return pipeline => database.collection(collection).aggregate(pipeline).toArray();
  return pipeline => {
...
beforeAll(async () => {
  ...
  mongoDb = mongoConnection.db('test');
});
...
describe('mongo memory servier is alive', () => {
  it('should start mongo server', async () => {
    ...
    expect(result).toMatchObject({ insertedCount: 2, acknowledged: true });
