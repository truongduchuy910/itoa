...
```javascript
const { Slug, Text } = require('@itoa/fields');
...
```javascript
const { Slug, Text, DateTime } = require('@itoa/fields');
const { Itoa } = require('@itoa/itoa');
...
```graphql
mutation {
  createPost(data: { title: "Why I ♥ ItoaJS" }) {
    id
    title
    url
  }
...
```graphql
mutation {
  createPost(data: { title: "Why I ♥ ItoaJS", url: "itoa-is-great" }) {
    id
    title
    url
  }
