...
export class SessionManager {
  ...
  constructor({
    ...
    if (!cookieSecret) {
      if (process.env.NODE_ENV === 'production') {
        throw new Error(
          'The cookieSecret config option is required when running Itoa in a production environment. Update your app or environment config so this value is supplied to the Itoa constructor. See [https://www.itoa.vn/itoa/itoa/#cookiesecret] for details.'
        );
      } else {
        console.warn(
          'No cookieSecret value was provided. Please generate a secure value and add it to your app. Until this is done, a random cookieSecret will be generated each time Itoa is started. This will cause sessions to be reset between restarts. See [https://www.itoa.vn/itoa/itoa/#cookiesecret] for details.'
        );
  ...
  getSessionMiddleware({ itoa }: { itoa: _Itoa }) {
    ...
    const _populateAuthedItemMiddleware = async (
      ...
      const item = await this._getAuthedItem(req, itoa);
      if (!item) {
  ...
  async _getAuthedItem(req: Request, itoa: _Itoa) {
