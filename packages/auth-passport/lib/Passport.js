 ...
 * - List: A Itoa 5 list (eg; User)
 * - Item: An item from a Itoa 5 list
 */
class PassportAuthStrategy {
  constructor(authType, itoa, listKey, config, ServiceStrategy = null, schemaName = 'public') {
    ...
    assert(
      ...
      'The `cookieSecret` config option for `PassportAuthStrategy` has been moved to the `Itoa` constructor: `new Itoa({ cookieSecret: "abc" })`.'
    );
    assert(
    ...
    this._sessionManager = itoa._sessionManager;
  ...
  async _executeQuery({ query, variables }) {
    const { errors, data } = await this._itoa.executeGraphQL({
      context: this._itoa.createContext({ schemaName: this._schemaName }).sudo(),
      query,
      variables,
    });
    if (errors) {
  ...
  // Called this from within Itoa's .prepare() method
  prepareMiddleware() {
    const app = express();
  ...
  _createSessionList() {
    if (!this._getSessionList()) {
      ...
      // TODO: Set read permissions to be 'internal' (within itoa) only so
      ...
      // reasons
      this._itoa.createList(PASSPORT_SESSION_LIST_KEY, {
        fields: {
  ...
  _getList() {
    return this._itoa.lists[this._listKey];
  }

  _getSessionList() {
    return this._itoa.lists[PASSPORT_SESSION_LIST_KEY];
  }
  ...
  async _createOrUpdateItem({ itemData, operation, passportSessionInfo }) {
    ...
    const data = await this._executeQuery({
      ...
      `,
      // Create the Itoa item as a Nested Mutation
