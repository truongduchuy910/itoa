...
// }
const generateInvalidateionPaths = memoizeOne(itoaTypes =>
  itoaTypes.reduce((memo, { schema: { type, queries = [], relatedFields = [] } }) => {
    memo[type] = [
      [typeToRegex(type)],
...
// the Apollo cache
const updater = (itoaSchemaInfo, types) => {
  const cacheInvalidationFieldPaths = generateInvalidateionPaths(itoaSchemaInfo);
  const paths = flatten(
...
class ItoaApolloQueryCacheBuster extends React.Component {
  state = {
    data: null,
    dataSeen: false,
    refetching: false,
...
// below ItoaMutation component for clearing caches
const ItoaQuery = ({ children, ...props }) => (
  <Query {...props}>
    {renderProps => <ItoaApolloQueryCacheBuster {...renderProps} children={children} />}
  </Query>
);

const wrapUpdateFunc = (itoaSchemaInfo, invalidateTypes, update) => (proxy, data) => {
  if (invalidateTypes) {
    updater(itoaSchemaInfo, invalidateTypes)(proxy, data);
  }
...
//    }}
//
// 3. Must be used within a tree wrapped in <ItoaProvider> or withItoa()
class ItoaMutation extends React.Component {
  render() {
    ...
    return (
      <SchemaContext.Consumer>
        {itoaSchemaInfo => {
          const interceptedUpdateProp = wrapUpdateFunc(itoaSchemaInfo, invalidateTypes, updateProp);
...
// Will contain the result of the META_QUERY above (see note in ItoaProvider
// for more info on when/why)
let itoaListsMeta = [];

class ItoaProvider extends React.Component {
  render() {
    return (
      <ItoaQuery query={META_QUERY}>
        {({ data, loading }) => {
          ...
          itoaListsMeta = loading || !data ? emptyArray : data._ksListsMeta || emptyArray;
          return (
            <SchemaContext.Provider value={itoaListsMeta}>
              {this.props.children}
...
// A HOC version of <ItoaProvider>
const withItoa = Component => {
  ...
  WrappingComponent.displayName = `WithItoa(${Component.displayName || Component.name})`;
...
function injectIsOptimisticFlag({
  ...
  const injectIsOptimisticFlagToItoaTypes = memoizeOne((inputResolvers, itoaTypes) =>
    (itoaTypes || []).reduce(
      (memo, { schema: { type } }) => ({
        ...memo,
        [type]: {
  ...
  return {
    ...
    resolvers: () => injectIsOptimisticFlagToItoaTypes(resolvers, itoaListsMeta),
    defaults: {
      ...defaults,
      _isOptimistic: false,
    },
