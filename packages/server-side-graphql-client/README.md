...
This library provides wrapper functions around `itoa.executeGraphQL` to make it easier to run server-side queries and mutations without needing to write boilerplate GraphQL.

Creating a new `User` item would be written as follows using `itoa.executeGraphQL`:
...
Using `@itoa/server-side-graphql-client` we can replace this with
...
const seedUsers = async usersData => {
  await createItems({ itoa, listKey: 'User', items: usersData });
};
```
...
| -------------- | -------- | ---------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `itoa`         | `Itoa`   | (required) | Itoa instance.                                                                                                                                                                                                      |
| `listKey`      | `String` | (required) | Itoa list name.                                                                                                                                                                                                     |
...
> NOTE: If `context` argument is provided then the `itoa` argument is not required.
...
const addUser = async userInput => {
  ...
  console.log(user); // { name: 'itoa user', email: 'itoa@test.com'}
};

addUser({ name: 'itoa user', email: 'itoa@test.com' });
```
...
Retrieve multiple items. Use [where](https://www.itoa.vn/guides/intro-to-graphql/#where) clause to filter results.

#### Usage

```js
const { getItems } = require('@itoa/server-side-graphql-client');
...
const getUsers = async () => {
  const allUsers = await getItems({ itoa, listKey: 'User', returnFields: 'name' });
...
| Properties | Type                             | Default | Description                                                                                                                                 |
| ---------- | -------------------------------- | ------- | ------------------------------------------------------------------------------------------------------------------------------------------- |
| `where`    | GraphQL `[listKey]WhereInput`    | `{}`    | Limit results to items matching [where clause](https://www.itoa.vn/guides/intro-to-graphql/#where).                                         |
| `sortBy`   | GraphQL enum `Sort[listKey]sBy}` |         | Returned the results based on specified order. Refer [docs](https://www.itoa.vn/guides/intro-to-graphql#sortby) for available sort options. |
...
```js
const { updateItem } = require('@itoa/server-side-graphql-client');
...
```js
const { updateItems } = require('@itoa/server-side-graphql-client');
...
```js
const { deleteItem } = require('@itoa/server-side-graphql-client');
...
const deleteUser = async itemId => {
  const user = await deleteItem({ itoa, listKey: 'User', itemId });
...
```js
const { deleteItems } = require('@itoa/server-side-graphql-client');
...
const deletedUsers = async items => {
  const users = await deleteItems({ itoa, listKey: 'User', items });
...
| Properties  | Type     | Default    | Description                                                                                                                                                                                                         |
| ----------- | -------- | ---------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `itoa`      | Object   | (required) | Itoa instance.                                                                                                                                                                                                      |
