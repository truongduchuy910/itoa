...
function Reset({ canReset, onReset }) {
  ...
  return resetRequested ? (
    <div css={{ display: 'flex', alignItems: 'center', marginLeft: gridSize }}>
      <div css={{ fontSize: '0.9rem', marginRight: gridSize }}>Bạn chắc chứ?</div>
      <Button className="btn btn-danger" autoFocus onClick={onReset} variant="ghost">
        Hủy thảo tác
      </Button>
      <Button
        variant="subtle"
      ...
      <Fragment>
        <Toolbar key="footer">
          <div css={{ display: 'flex', alignItems: 'center' }}>
            <LoadingButton
              className={`btn btn-${hasWarnings && !hasErrors ? 'warning' : 'primary'}`}
              id={cypressId}
              ...
              type="submit"
            >
              {hasWarnings && !hasErrors ? 'Bỏ qua cảnh báo và lưu thay đổi' : 'Lưu thay đổi'}
            </LoadingButton>
