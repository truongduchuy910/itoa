...
import { omit, arrayToObject, countArrays } from '@itoa/utils';
...
const UpdateManyModal = ({ list, items, isOpen, onUpdate, onClose }) => {
  ...
  return (
    <Drawer
      ...
      onClose={handleClose}
      closeOnBlanketClick
      heading={`Cập nhật ${list.formatCount(items)}`}
      onKeyDown={onKeyDown}
      slideInFrom="left"
      footer={
        <Fragment>
          ...
          <Button className="btn btn-warning" variant="subtle" onClick={handleClose}>
            Hủy
          </Button>
        </Fragment>
      }
